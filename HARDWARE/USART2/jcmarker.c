...
//写入JPEG文件必要的头信息
void jmkr_write_frame_head (jpeg_compress_info *cinfo)	  //frame 框架、画面、图片
{
  int i;
  UINT16 Lf, X, Y;
  UINT8  P, Nf, C, H, V, Tq;
  jutl_write_byte(0xFF, cinfo->output);
  jutl_write_byte(SOF0, cinfo->output);		  // basic dct
  Nf = cinfo->num_comp;						  //字节数？
  P  = cinfo->precision;					  //精度
  X  = cinfo->image_width;					  //宽
  Y  = cinfo->image_height;					  //高
  Lf = 8 + 3*Nf;
...
void jmkr_write_huff_table (jpeg_compress_info *cinfo)
{
  ...
  for (i=0; i<NUM_HUFF_TBLS; i++) 
  {
    ...
    /* marker */
    jutl_write_byte(0xFF, cinfo->output);
    jutl_write_byte(DHT, cinfo->output);	 //huffman table defination
    for (j=1; j<=16; j++) 
    {
