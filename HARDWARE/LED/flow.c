...
u8 flow_task(uint8_t * current_image,uint8_t * previous_image ,uint8_t * current_imaget,uint8_t * previous_imaget ,float get_time_between_images)
{
  ...
  //if(!en_klt)
	...
	qual[0] = compute_flow(previous_imaget, current_imaget, 0, 0,0, &tempx, &tempy , get_time_between_images);
	pixel_flow_x_sadt=firstOrderFilter(	tempx ,&firstOrderFilters[ACC_LOWPASS_X],get_time_between_images);
	pixel_flow_y_sadt=firstOrderFilter(	tempy ,&firstOrderFilters[ACC_LOWPASS_Y],get_time_between_images);
